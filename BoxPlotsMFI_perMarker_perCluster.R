...
GetMFIBoxplots = function(df, markers, ID, clusters, metadata, group) {
  ...
  #if(!require(c(dplyr,data.table,tibble,ggplot2, ggpubr,ggbeeswarm)){
  ...
  MFIScale = aggregate(df[,-c(ncol(df)-1,ncol(df))], by=list(ID=df[,"ID"], Clusters=df[,"Clusters"]), FUN=mean)
